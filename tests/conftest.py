...
def db_session():
    ...
    try:
        db_name = f'{config_db.db_name.get_secret_value()}.db'
    ...
    yield db_session
    sql_query = (
        f'DROP TABLE IF EXISTS {config_db.table_name.get_secret_value()};'
    )
    db_session.execute(sql_query)
    if connection:
        connection.close()
    os.remove(f'{config_db.db_name.get_secret_value()}.db')
